...
class InstrScr(Screen):
    def __init__(self, **kwargs):
        ...
        instr = Label(text=txt_instruction, text_size=(450, None), font_size='12sp')
    ...
    def next(self):
        self.in_name.text
        user_age = check_int(self.in_age.text)
        if user_age is False or user_age < 7:
            user_age = 7
            self.in_age.text = str(user_age)
        else:
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        instr = Label(text=txt_test1, text_size=(450, None))
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        instr = Label(text=txt_sits, text_size=(450, None), size_hint=(0.5, 1))
        self.lbl_sits = Sits(30)
        self.run = Runner(total=30, steptime=1.5, size_hint=(0.4, 1))
        self.run.bind(finished=self.run_finished)

        line = BoxLayout()
        v_lay = BoxLayout(orientation='vertical', size_hint=(0.3, 1))
        v_lay.add_widget(self.lbl_sits)
        v_lay.add_widget(instr)
        # line.add_widget(instr)
        line.add_widget(v_lay)
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        instr = Label(text=txt_test3, text_size=(450, None))
